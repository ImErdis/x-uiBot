...
async def group(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    client = {
        'name': ACCOUNT[f'{query.from_user.id}']['name'],
        '_id':
            str(idi),
        'servers': {},
        'usage_per_server': {},
    ...
    await query.edit_message_text(
        f"اسم اکانت: \n{ACCOUNT[f'{query.from_user.id}']['name']}\n\n" + '\n\n'.join(
            serv) + '\n\n🍩 لینک اشتراک: \n`' + subscrpt + '`', parse_mode='markdown'
    )
    resellers.update_one({'_id': user.id}, {
        '$inc': {'purchased_amount': int(reseller['ppg']) * client['traffic']}
    })
    ...
    await update.get_bot().send_message(config.admin, f"""✅ Account {client['name']} has been purchased by {user.full_name}_({user.id})_.
